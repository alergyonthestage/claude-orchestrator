...
| **Shareable, versioned config** | Commit `<repo>/.cco/` with `cco project save` — only the config, never the rest of your working tree; version your personal `~/.cco` store with `cco config save/push/pull`. Both secret-scan before committing, and `cco project history` / `cco config history` read either back. | [Configuration management](docs/users/configuration/guides/configuration-management.md) · [Configuring rules](docs/users/configuration/guides/configuring-rules.md) |
