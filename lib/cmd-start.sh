...
_start_generate_metadata() {
    ...
    [[ "${CCO_DEBUG:-}" == "1" ]] && echo "[debug] metadata: has_knowledge=$has_knowledge has_llms=$has_llms" >&2
    if [[ "$has_knowledge" == "true" || "$has_llms" == "true" ]]; then
        [[ "${CCO_DEBUG:-}" == "1" ]] && echo "[debug] metadata: writing to $packs_md" >&2
        ...
        if [[ "$has_knowledge" == "true" ]]; then
            ...
            while IFS= read -r pack_name; do
                ...
                [[ "${CCO_DEBUG:-}" == "1" ]] && echo "[debug] metadata: pack=$pack_name" >&2
                ...
                [[ "${CCO_DEBUG:-}" == "1" ]] && echo "[debug] metadata: got pack_files for $pack_name" >&2
                ...
                [[ "${CCO_DEBUG:-}" == "1" ]] && echo "[debug] metadata: pack $pack_name done" >&2
            done <<< "$pack_names"
        fi
