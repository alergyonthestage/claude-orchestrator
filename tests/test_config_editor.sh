...
# ── Mount-generation intent (RC-17 §3.6 rule; RC-1 + RC-6) ───────────
# The lane's mount-generation rule: every mount-generation fix asserts BOTH the
# line that must appear AND the line that must not. An "expected line present"
# assertion alone passes with the bug in place — the correct rw mount for the
# target's .cco IS emitted today and is then clobbered by a spurious self-match
# :ro line one line below it.
#
# ⚠ EXPECTED TO FAIL until RC-1 and RC-6 land (02-mount-generation.md,
# 03-config-editor-repos.md). Both defects are visible in ONE generated compose,
# so this is the shared failing reproduction for both stages, not a regression.
#
...
# unscoped escape-hatch fallback in _index_get_path then rescues the config-editor
# lookup and MASKS RC-6 — a test built on it passes before and after the fix. The
# target below therefore declares a repo bound ONLY per-project.
test_config_editor_lane_mount_intent_target_repo_and_no_self_match() {
    local tmpdir; tmpdir=$(mktemp -d); trap "rm -rf '$tmpdir'" EXIT
    setup_cco_env "$tmpdir"
    setup_global_from_defaults "$tmpdir"
    create_project "$tmpdir" "cave" "$(printf 'name: cave\ndescription: "t"\nrepos:\n  - name: cave\n')"
    cd "$tmpdir"   # neutral cwd: the target comes from --project, not from cwd
    run_cco start config-editor --project cave --dry-run --dump
    local compose="$DRY_RUN_DIR/.cco/docker-compose.yml"

    # The line that MUST appear (the target's committed .cco, writable) — present
    # today, which is exactly why it cannot be the only assertion.
    assert_file_contains "$compose" "$tmpdir/repos/cave/.cco:/workspace/cave-config" || return 1
    # RC-1: the nested-config discovery helper returns its own root (rel="."), so
    # it re-overlays the mount on top of itself :ro and clobbers the rw intent.
    assert_file_not_contains "$compose" "$tmpdir/repos/cave/.cco/.:/workspace/cave-config/.:ro" || return 1
    # RC-6: the target's own repo is never mounted, so ADR-0042 §8 repo-aware
    # authoring is declared but not delivered — the repo mount bridge derives its
    # project from the GENERATED config-editor manifest and misses the [cave] binding.
    assert_file_contains "$compose" "$tmpdir/repos/cave:/workspace/cave" || return 1
    return 0
}
