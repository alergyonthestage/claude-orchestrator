...
test_knowledge_section_generated() {
    ...
    decode_session_context "$compose" | grep -q "Read the relevant files BEFORE" || fail "knowledge section expected in context"
}
...
test_knowledge_path_format() {
    ...
    run_cco start "test-proj" --dry-run --dump
    local compose="$DRY_RUN_DIR/.cco/docker-compose.yml"
    decode_session_context "$compose" | grep -q -- "- /workspace/.claude/packs/my-pack/overview.md" || fail "knowledge path overview.md should be in the injected context"
    decode_session_context "$compose" | grep -q -- "- /workspace/.claude/packs/my-pack/conventions.md" || fail "knowledge path conventions.md should be in the injected context"
}
...
YAML
    ...
    run_cco start "test-proj" --dry-run --dump
    local compose="$DRY_RUN_DIR/.cco/docker-compose.yml"
    decode_session_context "$compose" | grep -q -- "- /workspace/.claude/packs/multi-file-pack/doc-a.md" || fail "knowledge path doc-a.md should be in the injected context"
    decode_session_context "$compose" | grep -q -- "- /workspace/.claude/packs/multi-file-pack/doc-b.md" || fail "knowledge path doc-b.md should be in the injected context"
    decode_session_context "$compose" | grep -q -- "- /workspace/.claude/packs/multi-file-pack/doc-c.md" || fail "knowledge path doc-c.md should be in the injected context"
}
...
YAML
    ...
    run_cco start "test-proj" --dry-run --dump
    local compose="$DRY_RUN_DIR/.cco/docker-compose.yml"
    decode_session_context "$compose" | grep -q -- "- /workspace/.claude/packs/pack-a/a-overview.md" || fail "knowledge path a-overview.md should be in the injected context"
    decode_session_context "$compose" | grep -q -- "- /workspace/.claude/packs/pack-b/b-guide.md" || fail "knowledge path b-guide.md should be in the injected context"
}
...
test_no_knowledge_section_when_no_packs() {
    ...
    assert_file_not_exists "$DRY_RUN_DIR/.claude/packs.md"
    # The injected context has no knowledge section when there are no packs
    if decode_session_context "$compose" | grep -q "Read the relevant files BEFORE"; then
        echo "ASSERTION FAILED: no knowledge section without packs"
        return 1
    fi
...
YAML
    ...
    run_cco start "test-proj" --dry-run --dump
    local compose="$DRY_RUN_DIR/.cco/docker-compose.yml"
    decode_session_context "$compose" | grep -q -- "- /workspace/.claude/packs/desc-pack/guide.md" || fail "knowledge path guide.md should be in the injected context"
    decode_session_context "$compose" | grep -q -- "packs/desc-pack/guide.md — Read when working on guides" || fail "knowledge description should render inline"
}
...
YAML
    ...
    run_cco start "test-proj" --dry-run --dump
    local compose="$DRY_RUN_DIR/.cco/docker-compose.yml"
    decode_session_context "$compose" | grep -q -- "- /workspace/.claude/packs/nodesc-pack/notes.md" || fail "knowledge path notes.md should be in the injected context"
    # A file with no description renders as a plain path line with no ' — ' suffix
    decode_session_context "$compose" | grep -qE -- "- /workspace/.claude/packs/nodesc-pack/notes.md\$" || fail "no-description knowledge renders path only"
}
...
test_workspace_yml_contains_project_name() {
    ...
    local compose="$DRY_RUN_DIR/.cco/docker-compose.yml"
    assert_file_contains "$compose" "PROJECT_NAME=my-project"
}
...
YAML
    ...
    run_cco start "test-proj" --dry-run --dump
    local compose="$DRY_RUN_DIR/.cco/docker-compose.yml"
    decode_session_context "$compose" | grep -q -- "- repo: repo-a at /workspace/repo-a" || fail "repo repo-a should be listed"
    decode_session_context "$compose" | grep -q -- "- repo: repo-b at /workspace/repo-b" || fail "repo repo-b should be listed"
}
...
YAML
    ...
    run_cco start "test-proj" --dry-run --dump
    local compose="$DRY_RUN_DIR/.cco/docker-compose.yml"
    decode_session_context "$compose" | grep -q -- "- pack: my-pack" || fail "pack should be listed"
}
...
YAML
    ...
    run_cco start "test-proj" --dry-run --dump
    local compose="$DRY_RUN_DIR/.cco/docker-compose.yml"
    decode_session_context "$compose" | grep -q -- "- repo: my-repo at /workspace/my-repo" || fail "repo my-repo should be listed with its path"
}
...
YAML
    ...
    run_cco start "test-proj" --dry-run --dump
    local compose="$DRY_RUN_DIR/.cco/docker-compose.yml"
    decode_session_context "$compose" | grep -q -- "- repo: my-repo at /workspace/my-repo — Seeded from project.yml" || fail "repo description should be seeded from project.yml"
}
...
YAML
    ...
    run_cco start "test-proj" --dry-run --dump
    assert_output_contains "no valid top-level keys"
    # The injected context knowledge section must NOT list files from the bad pack
    if decode_session_context "$DRY_RUN_DIR/.cco/docker-compose.yml" | grep -qFe "guide.md"; then
        echo "ASSERTION FAILED: injected context should not contain entries from bad-indentation pack"
        return 1
    fi
}
