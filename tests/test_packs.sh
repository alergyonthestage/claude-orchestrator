...
test_packs_md_file_is_created() {
    ...
    assert_file_exists "$DRY_RUN_DIR/.claude/packs.md"
}
...
test_packs_md_removed_when_no_packs() {
    # If project has no packs section, dry-run should not generate a packs.md
    # and the stale file in the project dir should be preserved (dry-run is side-effect free)
    local tmpdir; tmpdir=$(mktemp -d); trap "rm -rf '$tmpdir'" EXIT
    setup_cco_env "$tmpdir"
    setup_global_from_defaults "$tmpdir"
