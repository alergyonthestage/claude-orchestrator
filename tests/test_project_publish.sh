...
_create_empty_bare_remote() {
    ...
    # Structure-based sharing repo — no manifest.yml (ADR-0012/0018 D3); seed
    # only a marker so the bare repo is pushable.
    : > "$work_dir/.gitkeep"
...
test_project_publish_structure_based_on_remote() {
    local tmpdir; tmpdir=$(mktemp -d); trap "rm -rf '$tmpdir'" EXIT
    setup_cco_env "$tmpdir"
    setup_global_from_defaults "$tmpdir"
    _create_test_project "my-proj"
    ...
    # The template is published under templates/<name>/ and discovered by
    # structure — the sharing repo carries NO manifest.yml (ADR-0012/0018 D3).
    local verify_dir="$tmpdir/verify"
    git clone -q "$bare_dir" "$verify_dir"
    assert_file_exists "$verify_dir/templates/my-proj/project.yml" || return 1
