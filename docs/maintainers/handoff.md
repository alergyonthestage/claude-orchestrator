...
| Suite, impl branch | the implementer reported `tests/test_dev_protection.sh` **23/23** and `test_invariants` **42/42** green, and re-confirmed the baseline **1812 / 7 / 1819** mid-work. ⚠ **Its full-suite figure on the final tree was still running when the session closed and was never reported** — treat it as unmeasured |
| Suite, hooks-fix branch | **1813 passed / 7 failed / 1820**, `Results:` line **present**. Baseline + the one new invariant. ⚠ **6 of the 7 verified by name** (the `test_as_*` set); the 7th (`test_paths_symlink_safe_tool_root`) was not reached before the session closed — count matches, name-by-name is 6/7 |
