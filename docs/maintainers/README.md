...
| **naming** | Resource naming & rename: where each resource's name/ID is stored, the identity re-key model (generalizing `cco project rename`), and mono- vs multi-repo naming. | [naming/design/design-resource-rename.md](naming/design/design-resource-rename.md) · [naming/analysis/resource-name-storage-map.md](naming/analysis/resource-name-storage-map.md) · [naming/decisions/](naming/decisions/) |
