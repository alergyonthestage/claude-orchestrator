# Agent ↔ cco Access — End-to-End Validation Handoff (v1 — first review, done)

> **Phase status (2026-07-08)**: this **v1 review is complete** — it produced the seed/root
> findings (see [`results/`](results/)), which the fixes then addressed. The **re-validation**
> runs a **v2 acceptance handoff** written *after* the hardening-v2 design + implementation
> (unified permission model + the S1/S1b security fix); see the master plan
> [`../hardening-v2/handoff.md`](../hardening-v2/handoff.md). Kept here as the record of the
> first pass and a template for v2.
>
...
> **Path note — sessions on a project ≠ `claude-orchestrator`.** The `docs/…` paths above are
> repo-relative and resolve directly only in the `claude-orchestrator` sessions (S1, S5), where
> the repo is mounted. Every other session does **not** mount the `claude-orchestrator` repo;
> instead its `docs/` tree is bind-mounted read-only at **`/cco-docs`** (see §2 launch rule 3).
> There, read each reference at **`/cco-docs/maintainers/…`** — e.g.
> `/cco-docs/maintainers/configuration/agent-cco-access/design.md`. This handoff itself lives at
> `/cco-docs/maintainers/configuration/agent-cco-access/e2e-review/handoff.md`.
...
3. **Reference-docs mount (sessions on a project ≠ `claude-orchestrator`)**: those
   sessions do **not** mount the `claude-orchestrator` repo, so neither this handoff
   nor the design/ADR reference docs are reachable. Every such session — **S2, S3,
   S4, S6, S7, S8** — adds `--mount docs:/cco-docs:ro`, run from the
   `claude-orchestrator/` repo root so the relative `docs` source resolves against
   `$PWD`. This bind-mounts the whole `docs/` tree read-only at **`/cco-docs`**,
   making both the §1 reference docs (`/cco-docs/maintainers/…`) and this handoff
   (`/cco-docs/maintainers/configuration/agent-cco-access/e2e-review/handoff.md`)
   available in one shot. **S1 and S5** run on `claude-orchestrator` (repo mounted,
   docs in-repo) and need no docs mount. **Do not** hand-copy the handoff into
   `/review` — this docs mount supersedes that workaround.
4. **Review-only**: sessions may *test-write* to disposable targets to exercise
...
5. **Parallelism**: sessions on distinct projects run concurrently. Same-project
...
> in a `claude-orchestrator` session (S1/S5), or
> `/cco-docs/maintainers/configuration/agent-cco-access/e2e-review/handoff.md` in any other
> session (docs mounted per §2 launch rule 3) — then execute **section S<N>** end-to-end:
> run the shared checklist (§3) + the section focus, hunt for same-class issues beyond the
> known seeds, and write your findings to `/review/S<N>-<slug>.md` using the §4 template.
...
| S2 | `readproject-rich` | `./bin/cco start cave-auth --mount ~/cco-e2e-review:/review:rw --mount docs:/cco-docs:ro` | rich: 3 repos, packs cave-core+cave-web | read-project scoping **with** referenced packs |
| S3 | `readglobal-rich` | `./bin/cco start cave-auth --cco-access read-global --mount ~/cco-e2e-review:/review:rw --mount docs:/cco-docs:ro` | rich (A/B vs S2) | full-read: no scoping, all kinds visible, no notice |
| S4 | `none-mid` | `./bin/cco start cave-eda-flow --cco-access none --mount ~/cco-e2e-review:/review:rw --mount docs:/cco-docs:ro` | mid: 2 repos, pack cave-core | no-cco: Level-A without wrapped cco; CLI absent |
...
| S6 | `configeditor-broad` | `./bin/cco start config-editor --mount ~/cco-e2e-review:/review:rw --mount docs:/cco-docs:ro` | all projects' `.cco`, no repos | edit-all broad; cross-project view |
| S7 | `configeditor-project` | `./bin/cco start config-editor --project cave-auth --mount ~/cco-e2e-review:/review:rw --mount docs:/cco-docs:ro` | cave-auth `.cco` + its 3 repos | repo-aware authoring (ADR-0042 §8) |
| S8 | `tutorial` | `./bin/cco start tutorial --mount ~/cco-e2e-review:/review:rw --mount docs:/cco-docs:ro` | internal tutorial | preset resolution (none/read-project teacher) |
...
- **Host**: `./bin/cco start cave-auth --mount ~/cco-e2e-review:/review:rw --mount docs:/cco-docs:ro`
...
- **Host**: `./bin/cco start cave-auth --cco-access read-global --mount ~/cco-e2e-review:/review:rw --mount docs:/cco-docs:ro`
...
- **Host**: `./bin/cco start cave-eda-flow --cco-access none --mount ~/cco-e2e-review:/review:rw --mount docs:/cco-docs:ro`
- **Mounts**: repos cave-flow, cave-eda-framework (rw); pack cave-core (ro); **no wrapped `cco`** (operator shim absent at `none`); `docs/` at `/cco-docs` (ro reference material — a plain bind-mount, independent of `cco_access`, so it is present even though the wrapped `cco` is not).
- **Access**: cco_access=none.
- **Focus**: Level-A must still fully inform the agent **without** any CLI — repos, packs, llms, path_map, and the awareness that `cco` is *not* available here. Confirm `cco` is genuinely absent/refused (no partial surface). This is the "awareness without operation" class: the whole burden is on injection (A) + managed rules (C). The `/cco-docs` mount is only how this session reaches the handoff + reference docs; it does **not** confer any `cco` capability.
...
- **Host**: `./bin/cco start config-editor --mount ~/cco-e2e-review:/review:rw --mount docs:/cco-docs:ro`
...
- **Host**: `./bin/cco start config-editor --project cave-auth --mount ~/cco-e2e-review:/review:rw --mount docs:/cco-docs:ro`
...
- **Host**: `./bin/cco start tutorial --mount ~/cco-e2e-review:/review:rw --mount docs:/cco-docs:ro`
