...
### Non-blocking — ✅ RESOLVED (maintainer, 2026-07-19, second round)

Settled immediately before implementation started, so the implementer never has to guess.
Binding on the clusters exactly as D-M1…D-M6 are.

| # | Answers | Resolution |
|---|---|---|
| **D-M7** | §4 of the handoff | **Cycle 1 only.** Cycle 2 (the RC-5 sweep and RC-7…RC-16) is *not* folded in. The incremental `review → fix → re-review` gate is kept intact so a re-review failure stays attributable to a specific fix. Cost accepted: two build/review rounds instead of one. |
| **D-M8** | Q-10, Q-11, Q-13, Q-14 | **Three scope enlargers are IN, one is OUT.** IN: **Q-13** supersede `_op_cco`/`_op_seed` with the lane in one pass (~35 call sites — two vocabularies for one context is how `_ps_probe` was born, and `_op_cco`'s missing `CCO_ACCESS_TRIPLE` unset is a live latent bug); **Q-11** migrate `repo`/`extra-mount rename` onto RC-3's primitive layer now (they are whole-verb elevated today — the same INV-S5 over-elevation — and they must be sequenced with RC-2 in the same file anyway); **Q-14** the `chmod 000` boundary seam in the lane, with the mandatory `id -u` self-check that fails loudly rather than skipping when run as root. OUT: **Q-10** — only the destructive/re-key ops convert; the provenance writers (`pack`/`template install\|update\|import\|internalize`, `llms install\|update`, `_record_tree_as_base`, `_meta_record_provenance`) defer to cycle 2, which makes `pack install` a **clean in-container refusal** until then. |
| **D-M9** | Q-4…Q-9 | **The designs' proposals are adopted en bloc.** Q-4: a named `not mounted in this session` target exits **2** (a refusal carrying a remedy, per B6). Q-5: `--move-dir` in operator mode is **refused, exit 2**, with a host hint — explicit user intent is never silently downgraded (ADR-0050 D4); the lane asserts the refusal (closes `01-test-lane.md` §8.3). Q-6: bare `cco repo rename <new>` at the WORKDIR root is **always refused as ambiguous**, with no single-repo fallback. Q-7: two `--project` targets binding the same repo NAME → **mount the first, announce the second** (no disambiguated container path in cycle 1). Q-8: the **duplicate authoring path is accepted** — no `:ro` re-overlay of the repo-path copy; ADR-0046 §6 stays deferred. ⚠ **Superseded by `03-config-editor-repos.md` §3.7 — this Q-8 clause is stale: the implementation re-overlays `:ro` and is strictly safer.** §3.7 ratifies `_committed_ro=":ro"` for the config-editor built-in *regardless of `Pc`* (`cmd-start.sh:1602`), because accepting the duplication would hand a `Po=none` session read-write access to a **foreign** member repo's committed `.cco` — the `foreign` role `_project_member_role` explicitly classifies. What stays deferred is only what §3.7 says stays deferred: the general ADR-0046 §6 multi-repo `Pc` span for *normal* sessions. Q-9: a project-less config-editor global session gets the **honest empty `path list` + notice**, with no special case for the authoring tool. |
| **D-M10** | Q-12, Q-15, Q-16, Q-C1…Q-C4 | **The remaining minor questions, resolved as recommended.** Q-12: the `_setup_internal_config_editor` `readonly: false` latent defect **defers to cycle 2** — it is unreachable in every shipped mode, and D-M3 governs. ⚠ **Superseded by D-M11 — this Q-12 clause is wrong: D-M1 makes it reachable.** Q-15: the `dummy-repo` unscoped-seed conversion **defers to cycle 2**; RC-6 routes around it with an unseeded name and the 3-arg scoped form. Q-16: the owner-less pin is seeded **unconditionally** (largely moot under D-M8's Q-13 supersession). Q-C1: **one grouped changelog entry**, id **46**, owned by the docs stage. Q-C2: the ADR-0043 **forward annotation suffices** — no standalone ADR. Q-C3: notice unification **stays deferred**; `path list` keeps its dedicated notice. Q-C4: **T3 lands with the lane** matching the validator's current success line, and RC-2's stage tightens it to D-M2's settled string — sequencing only. |
| **D-M11** | Supersedes D-M10's Q-12 clause | **Q-12 lands in cycle 1 — it is a FORCED CONSEQUENCE of D-M1, not an independent scope enlargement.** The RC-1 implementation surfaced, and the adversarial verifier confirmed (test T15), that D-M1's `-mindepth 1` removes the nested-config self-match `/.:…:ro` line, which was the **only** thing enforcing `Pc=ro` on the config-editor target mount root. Deferring Q-12 while landing D-M1 therefore ships a real, reachable declared-vs-enforced **privilege escalation** (opposite polarity to the review's other findings): `cco start config-editor --cco-access global=rw,current=ro,others=none` would mount the current project's `.cco` **writable while `Pc=ro`**. D-M10's premise ("unreachable in every shipped mode") was true only *because* the self-clamp accidentally enforced it; D-M1 removes that accident. The fix — `02-mount-generation.md` §3.5, now **ratified** (maintainer, 2026-07-19) — derives the target mount root's `readonly:` from `Pc` via `_config_editor_mount_ro pc` (`cmd-start.sh:151,166`), the same source the store mount already uses. A bare-revert of §3.5 is **rejected**: it keeps D-M1's self-clamp removal and ships the escalation. ~3 lines, already implemented and tested with RC-1. Cycle-2 residue: none — Q-12 is closed. |
