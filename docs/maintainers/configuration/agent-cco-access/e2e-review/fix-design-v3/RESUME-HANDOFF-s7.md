...
> ⚠ **SUPERSEDED 2026-07-21 by [`RESUME-HANDOFF-s8.md`](RESUME-HANDOFF-s8.md).** S7 landed
> (`097ef61`), so §3 is history — including its item 2, which the maintainer settled as **(b)**.
> Everything else (§1 prerequisites, §2's seven invariants and three process lessons, §5's carried
> debt, §6's gates, §7's do-not-re-litigate list) is carried forward verbatim into `-s8`.
> One correction this document contains: §3 item 1's fix site is **dead code** — see `00-plan.md`
> §8's landed note.
