...
| `workspace.yml` | Generated by `cco start` | Structured project summary + knowledge/llms file lists (injected via hook) |
...
cco start <project>
  ...
  ├── 2. Generate workspace.yml (knowledge/llms file lists + project summary)
  └── 3. session-context.sh injects the workspace.yml knowledge/llms sections into additionalContext
```
