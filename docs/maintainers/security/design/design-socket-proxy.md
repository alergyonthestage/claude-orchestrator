...
services:
  ...
  cco-proxy:
    ...
    volumes:
      # Machine-local CACHE managed dir (not the committed repo)
      - ${session_cache_dir}/managed/proxy/squid.conf:/etc/squid/squid.conf:ro
      - ${session_cache_dir}/managed/proxy/allowed_domains.txt:/etc/squid/allowed_domains.txt:ro
    networks:
...
- **Generated at**: `<cache>/cco/projects/<name>/managed/policy.json` (machine-local CACHE, `<cache>` = `~/.cache/cco`)
- **Mounted at**: `/etc/cco/policy.json:ro`
- **Generated by**: `cmd-start.sh` (into the per-session CACHE managed dir, `$managed_gen_dir`)

### 7.3 Squid Config Location (Phase C — planned, network isolation still pending)

- **Generated at**: `<cache>/cco/projects/<name>/managed/proxy/squid.conf` and `allowed_domains.txt` (machine-local CACHE)
...
volumes:
  ...
  # Policy config (machine-local CACHE managed dir, not the committed repo)
  - ${session_cache_dir}/managed/policy.json:/etc/cco/policy.json:ro
```
