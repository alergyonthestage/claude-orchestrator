# Roadmap — History & Detailed Records

> **Status**: historical record (chronology); the live plan is in roadmap.md
>
> This file preserves the full chronological record of completed work, the detailed
> decentralized-config build narrative, the known-bug log, and the verbatim long-form
...
- [Planned Sprints — detailed descriptions](#planned-sprints)
...
## Decentralized-config refactor — phase-by-phase log

Navigable summary of the implementation chronology. Source of truth for design is
`configuration/decentralized-config/{design.md,requirements.md}` + ADRs 0005–0027.
Branch: `feat/vault/decentralized-config` (commits local; maintainer pushes from Mac).

### Design & review closure (pre-implementation)

- **Config + sharing analyses** — RD-* + R1–R4 + Cat-4 + M + S resolved into ADRs
  0005–0023; 4-bucket taxonomy (CONFIG/DATA/STATE/CACHE), coordinate-per-unit model,
  sharing unification (2×2 matrix, pack coordinates, reachability, working-copy
  lifecycle, permissions delegated-to-git), principles P1–P17.
- **Impl-readiness review (V), 2026-06-18** — `reviews/18-06-2026-impl-readiness-review.md`,
  58 findings / 37 decisions, resolved cluster by cluster (all 5 closed 2026-06-19):
  Cluster 1 (migration safety) → ADR-0021 + ADR-0006/0009/0010; Cluster 2 (phasing &
  test plan) → 6-phase dependency-layer map (P0–P5); Cluster 3 (doc resync) Block A →
  `.claude/rules/documentation-lifecycle.md`; Cluster 4 (coordinate model) → ADR-0022;
  Cluster 5 (command surface & UX) → ADR-0023.

### Phase 0 — substrate (✅ CLOSED 2026-06-21)

T1 resolver `ff8278b`, T2a index API `d913e5c`, T3 coordinate parsers `992738d`,
T4-remotes M3 split `2bdf80e`, Commit A `c8ae080` (repos/mount → STATE index), Commit B
`848cf63` (session-mount bucket re-point + harness HOME flip), T8 `7dcf1e8` (`.claude`
overlays → CACHE `:ro`). Adherence audit 2026-06-21 (`reviews/21-06-2026-impl-adherence-review.md`);
runner-masking fix un-masked 17 hidden failures → re-baselined **982/16**.

### Phase 1 — core local (✅ CLOSED 2026-06-22)

6 commits `56ca45c`→`e48abdd`. `cco resolve`/`cco path` (index-backed, `--scan`
upsert, clone-from-url), sync-meta fingerprint (§4.6/F39), non-blocking reminder
aggregator (ADR-0008), `cco sync` (copy), `cco start` aggregator, `cco project add
repo|mount|llms|pack`. Suite **1043/16**. P1 adherence audit clean.

### Phase 2 — migration & bootstrap (✅ CLOSED 2026-06-22)

5 commits `c1e0369`→`767de86`. J0 bootstrap + raw-tar backup, H6 base/meta → STATE
keyed-by-name + global-meta decompose (16→8), eager global migration via `cco update`,
`cco init --migrate` lazy + `cco join` + migration 013, D5 observability. Suite **1087/8**.
ADR-0024 (`RD-repo-multi-project`, principle P18), ADR-0025 (P2 design).

### Phase 3 — legacy cutover (✅ CLOSED 2026-06-24)

P3-1…P3-5: decentralized `cco start` read-path, `cco tag`/`cco list` over DATA
`tags.yml`, `cco config save/push/pull`, the **vault/profile world REMOVED**
(`cmd-vault.sh` −3732), `cco init` scaffold (ADR-0026), config-editor built-in moved to
`internal/config-editor/` (ADR-0027), shipped-behavior doc cutover sweep + `_archive/`.
Suite **936/3**.

### Phase 4 — sharing core (✅ CLOSED 2026-06-24)

P4-1 source→DATA + key rename (`82b6956`, ADR-0022 D1), P4-2 structure-based discovery +
**manifest subsystem deleted** (`6b2673f`), P4-3 sync-before-publish 3-way merge
(`cf8d03b`, ADR-0022 D5), P4-4 2×2 verb wiring (pack/template/project export-import +
`cco init --template`; project publish/install/update/internalize **removed**), P4-5a/b/c
(tier-2 verbs removed · `@local` plumbing deleted · schema bridge collapsed to
index-only), P4-doc. Suite **827/1**. P4-5d (central `$PROJECTS_DIR`/`CCO_*_DIR`
teardown) deferred → P5. P4→P5 adherence audit clean.

### Phase 5 — sharing-ext + lifecycle (✅ BUILD COMPLETE 2026-06-25)

P5-0 llms name-fix `2f93de8` (baseline → **828/0**), P5-1 P4-5d central-layout teardown,
P5-2 `cco forget` + `cco config validate` + delete-cascade (ADR-0021), P5-3 three-layer
pack resolution + internalize + `export --bundle-packs` + template-vars (ADR-0019),
P5-4 `cco project validate` + `cco project coords`, P5-5 `cco update --check` 3-state
(packs+templates only), P5-6 `cco config protect` (governance docs only; helper deferred
post-v1, ADR-0023 D6), P5-doc (changelog #15 + migration-check = NONE + browser-mcp reader
sweep). Suite **894/0**.

### Pre-merge review cycle (in progress)

- **Step 1 — impl-adherence review** DONE 2026-06-25 → pre-merge fix backlog F1–F5
  RESOLVED (baseline **897/0**, merge-gate cleared).
- **Migration / decentralized-config implementation review** (deep) DONE 2026-06-26 →
  `reviews/26-06-2026-migration-impl-review.md` (2 BLOCKER + 7 HIGH + 10 MEDIUM + LOW/NIT);
  ALL FINDINGS RESOLVED 2026-06-26 (19 commits `d136344`→`738973e`). H7 = real latent
  bug (migrated memory written to `projects/<id>/memory` but mounted from
  `projects/<id>/session/memory`). Baseline **905/0**.
- **Step 2 — documentation review**: NEXT (this work), pending maintainer go-ahead.
- ADRs **0005–0027**; next free **0028**.
...
#### Pre-merge fix backlog (decentralized-config) — ✅ RESOLVED 2026-06-25
...
merge-ready — 1 HIGH + 3 MEDIUM 🔴 + 1 MEDIUM ❌; 0 blockers; 13 optimization flags (flag-only). **All
five fixes applied in a dedicated implementation session (2026-06-25), each delta-green; new baseline
897/0 (+3 pinning tests). The branch merge-gate (F1) is cleared.** Commits LOCAL (maintainer pushes from
Mac); the §4 Transitional Registry LIVE set stays empty.
- **F1 (HIGH, gate) ✅** `392f12e` — relocated the personal flat store: `packs`/`templates` →
  `~/.cco/{packs,templates}` (CONFIG), `llms` content+cache-state → CACHE via the new `_cco_llms_dir`.
  `USER_CONFIG_DIR` kept ONLY as the legacy-vault pointer + internal tutorial/config-editor runtime root;
  removed the central-layout remnants (`update.sh` migration-003 fallback, the `cmd-remote`
  `$USER_CONFIG_DIR/.git` mirror, a dead `cmd-pack` cross-branch check calling an undefined function);
  fixed the false `paths.sh` comment; aligned the harness. **No relocation migration** (maintainer-
  confirmed): the branch is unmerged with no users on the broken path, the eager global migration already
  targets `~/.cco`, and llms is re-fetchable into CACHE.
- **F5 (MEDIUM, test-isolation) ✅** `72a6253` — one `CCO_FRAMEWORK_ROOT` seam (defaults/templates/
  migrations/changelog derive from it); the harness sandbox-copies those assets and `with_framework_change`
  mutates the copy (no trap restore). **Scope broadened (maintainer-confirmed)** beyond the 2 enumerated
  sites to ALL tracked-file mutations (the ~15 `with_framework_change` sites, `test_merge` bare appends,
  the 999 migration) → the suite is now genuinely concurrency/abort-safe.
- **F3 (MEDIUM, doc) ✅** `d641528` — removed the `cco template update` example; added a 🚧 note.
- **F2 + F4 (MEDIUM, tests) ✅** `ab64f1b` — F4 P15 discriminator (url ⇒ cache vs url-less ⇒ authored,
  incl. the §2.4 ERROR collision) in `test_project_validate.sh` (the discriminator's authority — `cco
  project validate` — not the url-agnostic mount resolver); F2 H1 ordering in `test_start_reminders.sh`,
  verified via a negative control. (Review named `test_pack_resolution.sh`/`test_start_*`; re-homed for
  coherence with the existing collision/reminder tests.)

**Next = review-cycle step 2: documentation review** (pending maintainer go-ahead — do not auto-advance).
**Launcher = `../configuration/decentralized-config/documentation-review-handoff.md`** (self-contained:
doc-lifecycle law · whole-doc-landscape scope · the shipped-behavior cutover sweep now due · known
candidate work-list [browser-mcp deep rewrite · roadmap §73 reorg · F1 homes propagation · re-verify the
P3-5 user-facing docs post-P4/P5/F1 · stale `user-config/` dev artifacts · cross-ref/terminology] · method ·
out-of-scope). Then step 3 refactoring/optimization [consumes the 13 flags] → step 4 UX-UI → dogfooding e2e
on Mac (`../configuration/decentralized-config/P2-dogfooding-validation.md`) → merge/release v1.
...
## Block A — the A5 + A8 cycle (closed 2026-08-18, merged 2026-08-21)

> **Moved here whole and verbatim** from [roadmap.md](roadmap.md) at the merge, per the docs rule that
> the roadmap holds *where we are*, not the history of a closed cycle. Nothing below was edited in the
> move; one line each now stands in the roadmap, pointing here. The cycle shipped as five units on
> `feat/cli/start-warning-gate` and merged as `6208228` (`--no-ff`), suite **1710 / 7 of 1717** on the
> merged `develop`.
