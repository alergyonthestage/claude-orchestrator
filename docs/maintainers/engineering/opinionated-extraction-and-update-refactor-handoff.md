...
- **`settings.json` (global) is mixed** → must be **decomposed**, not moved wholesale (permissions
  allow-list, `teammateMode:tmux`, cleanup/thinking/attribution). Split: opinionated-and-user-editable vs
  functional-and-cco-managed/immutable. (Shared with Handover C §7.6.)
...
3. **`settings.json` decomposition** (shared with Handover C §7.6): functional/immutable vs opinionated.
