...
> rebuilt image — boundary/preset fixes live). **▶ e2e v2 is now DEFERRED to after the access-model
> refinements** (maintainer decision 2026-07-11: run acceptance on the final design) — see the
> "Access-model refinements" subsection. Pre-merge open: **B-DF1**, config-editor `--project` host
> live-dogfood, Linux write-path check-in; push both branches (from the Mac) after refinements.
