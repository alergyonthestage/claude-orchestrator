...
| **project-rename + resource naming fixes** | 📐 DESIGN DONE (2026-07-14) → IMPL next | Add per-kind `rename` for the kinds that lack it (**repo · extra-mount · pack · template · remote**; `project`/`llms` exist), + fix UX (quote-stripping in path input; name↔path divergence hint). **Decisions** ([ADR-0050](naming/decisions/0050-resource-rename-model.md), generalizes ADR-0031): per-kind verbs (no top-level `cco rename`); kind-scoped re-key (never rename a same-named resource of another kind); shared-repo cross-project fan-out with ADR-0031 D3 strictness; repo/extra_mount dir-move opt-in (basename-gated, default No); shared `lib/rename.sh`; operator-shim write-scope gating (repo→edit-project, pack/template/remote→edit-global). Refs: [design](naming/design/design-resource-rename.md) · [storage map](naming/analysis/resource-name-storage-map.md). Changelog additive, migration none. |
