...
| **D3** — A1 per-command info × scope | **▶ NEXT (clean session)** — kickoff [`D3-handoff.md`](configuration/agent-cco-access/hardening-v2/D3-handoff.md) | Per-verb classification by resource area (G/Pc/Po read/write, keyed off [ADR-0046](configuration/agent-cco-access/decisions/0046-unified-cco-access-model.md) §7) + which side of the [ADR-0047](configuration/agent-cco-access/decisions/0047-config-access-enforcement.md) helper boundary each verb sits; tag gating **B5**, hint invariant **B6**, `path` decision, `cco sync`-of-divergent-members (ADR-0046 §6), coverage gaps (`cco state`/`whoami`) | ADR-0046 §7/§6, ADR-0047, CLI-surface matrix, `bin/cco:_cco_operator_shim`, `lib/tags.sh`, `lib/paths.sh` |
