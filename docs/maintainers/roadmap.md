...
`--dry-run --dump → .cco/.tmp/`) landed next, suite **1010/0**. **Step 6 is satisfied; v1 is ready for
the merge to `develop`/`main` (step 7).** Intentional post-v1 deferrals (not blockers): the
...
| **C** | **npm packaging & distribution** — ship `cco` as an npm package (`package.json` `bin`, framework tree bundled, Docker/proxy at runtime, version coupling). **ADR-0037 ✅ written + living design doc.** **Priority.** **Design ready 2026-06-30**: pkg = **`@claude-orchestrator/cco`** (unscoped `claude-orchestrator`/`cco` both taken — registry-verified; dedicated npm+GitHub orgs); build-in-image proxy; `release.sh` + CI-on-tag (npm token in CI) + npm-pack hygiene gate; global prefix-free `cco`; `files` allowlist (`docs/users` ships, `config/proxy/Dockerfile` ship, `user-config/` excluded); the one read-only `FRAMEWORK_ROOT` violation (`USER_CONFIG_DIR`) **split** into legacy-source vs STATE runtime + read-only test gate; docs single-source `docs/users` → pkg + `cco docs` + Pages renderer. **▶ next = implementation.** | post-merge, on `develop` | **Release gate** | [`npm-packaging-distribution-handoff.md`](../engineering/npm-packaging-distribution-handoff.md) · [ADR-0037](../engineering/decisions/0037-npm-packaging-distribution.md) · [design](../engineering/design/packaging-distribution.md) |
