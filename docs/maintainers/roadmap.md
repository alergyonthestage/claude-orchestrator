...
| **Unit A — per-project name scoping (H7)** | 📐 DESIGN DONE (2026-07-14) → IMPL next (**do first**) | Make repo/extra_mount names **per-project scoped**; identity = **path**, name = per-project label ([ADR-0051](naming/decisions/0051-per-project-name-scoping.md), supersedes ADR-0022 D2). Scoped index (`project_paths`), `_index_path_conflicts` project-aware chokepoint, add-time disambiguation prompt (reuse same-path vs new path) + on-disk-url divergence warn, path-based reverse lookup replacing name-based. **Breaking** (index `version` 2, deterministic migration). ~32 index call-sites rethreaded (analysis §13.1). Refs: [ADR-0051](naming/decisions/0051-per-project-name-scoping.md) · [analysis §12–§13](naming/analysis/resource-name-storage-map.md). |
