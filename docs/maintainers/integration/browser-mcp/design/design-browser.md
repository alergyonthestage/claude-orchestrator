...
> **Status (current layout)**: managed MCP overlays (`browser.json`, `github.json`)
> and the `.browser-port` runtime file are generated per session into CACHE under
> `<cache>/cco/projects/<name>/managed/` (resolved by `_cco_project_cache_managed`);
> the whole `managed/` dir is mounted `:ro` at `/workspace/.managed`. Projects are
> enumerated via the STATE index (`_index_list_projects` / `_index_get_path`), not a
> central `projects/` dir. Port-conflict auto-assignment with warning (D8/D9), the
> `.browser-port` file, the `cco chrome --project` flag, and the Chrome 145+ Host
> header fix via socat CDP proxy (D10) are reflected throughout. The body below
> reflects this layout.
...
- Auto-generated `browser.json` managed MCP overlay injected at `cco start`
...
### D3: Separate managed `browser.json`, user's `mcp.json` untouched

The browser MCP configuration is framework-managed (auto-generated by `cco start`,
like `.cco/docker-compose.yml`). It must not be merged into or overwrite the user's
`mcp.json`, which may contain custom MCP servers.

Solution: generate a dedicated `browser.json` in the per-session managed cache dir
(`<cache>/cco/projects/<name>/managed/`), mount the whole `managed/` dir as a
read-only MCP source at `/workspace/.managed`, and merge every
`/workspace/.managed/*.json` in `entrypoint.sh`. The managed cache dir is
machine-local (CACHE, outside the repo), so nothing needs gitignoring.
...
`<cache>/cco/projects/<name>/managed/.browser-port` (resolved by
`_cco_project_cache_managed`) at `cco start` time and deleted by `cco stop`.

This file is:
- Machine-local in CACHE, outside the repo (no gitignore needed)
...
- `entrypoint.sh` starts `socat TCP-LISTEN:<port>,fork,bind=127.0.0.1,reuseaddr
  TCP:host.docker.internal:<port> &` when `/workspace/.managed/browser.json` is present
...
is always written to `<cache>/cco/projects/<name>/managed/.browser-port` (see D9).
...
sequenceDiagram
    ...
    cco->>Host: write managed/browser.json (effective port)
    cco->>Host: write managed/.browser-port (effective port)
    cco->>Host: generate .cco/docker-compose.yml (extra_hosts)
...
```
<repo>/.cco/                          ← committed project config (in the repo)
...
<cache>/cco/projects/<name>/managed/  ← generated per session (CACHE), mounted :ro at /workspace/.managed
├── browser.json                      ← auto-generated by cco start (managed MCP overlay)
└── .browser-port                     ← runtime: effective CDP port

# docker-compose.yml is generated into STATE and overlaid :ro — never committed.

Container:
~/.claude.json                        ← merged at startup: global + project + managed MCPs
/workspace/.mcp.json                  ← bind-mounted from <repo>/.cco/mcp.json
/workspace/.managed/browser.json      ← bind-mounted from <cache>/cco/projects/<name>/managed/
```
...
# simply never mount the browser.json that references it.
...
#### Generate `browser.json` and `.browser-port`
...
if [[ "$browser_enabled" == "true" ]]; then
    mkdir -p "$managed_gen_dir"   # <cache>/cco/projects/<name>/managed
...
#### `docker-compose.yml` generation — managed integrations mount

In the `volumes:` section of `services.claude:`, after the project MCP mount block,
mount the whole managed cache dir read-only (covers `browser.json` plus any other
managed overlays such as `github.json` / `policy.json`):
...
Merge every managed MCP config from the `/workspace/.managed` mount, after the
existing project MCP merge. The loop is generic — adding a new managed integration
(`github.json`, etc.) needs no change here:
    ...
    for mcp_file in /workspace/.managed/*.json; do
        ...
        if [ "$server_count" -gt 0 ]; then
            merged=$(jq -s \
                ...
                "$CLAUDE_JSON" "$mcp_file" 2>/dev/null) \
...
The guard `if [ -d "/workspace/.managed" ]` makes it a no-op for projects without
any managed integration enabled.

After the merge, the entrypoint starts a socat TCP proxy for the Chrome 145+ Host
header fix (see D10), guarded on the managed browser config:
...
### 5.5 `lib/cmd-stop.sh` — cleanup managed runtime state

When a project session is stopped, delete the generated managed runtime state from
CACHE (resolved by `_cco_project_cache_managed`, keyed by project name):
...
The managed overlays (`browser.json`, `.browser-port`) are generated into the
machine-local CACHE bucket (`<cache>/cco/projects/<name>/managed/`), outside any
repo, so they are never tracked by git and no `.gitignore` entries are required.

---

## 6. Generated Files

### `<cache>/cco/projects/<name>/managed/browser.json` (host mode)
...
services:
  claude:
    ...
    volumes:
      # ... existing volumes ...
      - <cache>/cco/projects/<name>/managed:/workspace/.managed:ro   # NEW
    extra_hosts:                                                       # NEW
      - "host.docker.internal:host-gateway"                           # NEW
```

No new services. No port changes. The managed-dir mount is the only volume
addition.

### `<cache>/cco/projects/<name>/managed/.browser-port`
...
2. `--project <name>` → reads `<cache>/cco/projects/<name>/managed/.browser-port` (effective runtime port)
3. `--project <name>` → falls back to the project's `<repo>/.cco/project.yml` `browser.cdp_port` (repo resolved via the STATE index)
4. Default: `9222`
...
| Existing `mcp.json` in project | Untouched: browser config goes to separate managed `browser.json` |
| Entrypoint on project without browser | The `if [ -d "/workspace/.managed" ]` guard is a no-op |
| `cco start` without `--chrome` | No change in behavior |
| `cco stop` on project without browser | `rm -f` of the managed runtime files is a no-op if absent |
...
|------|-----------------|
| `test_browser_disabled_by_default` | No `extra_hosts`, no `browser.json`, no browser service when `browser:` section absent |
...
| `test_browser_mcp_json_generated` | `browser.json` file created in the managed cache dir with correct content |
| `test_browser_mcp_mounted_in_compose` | the managed-dir mount (`…/managed:/workspace/.managed:ro`) appears in compose volumes |
| `test_browser_custom_cdp_port` | Custom `cdp_port: 9223` results in `--browserUrl=http://host.docker.internal:9223` in `browser.json` |
| `test_browser_chrome_flag_override` | `--chrome` flag on `cco start` activates browser even without `browser.enabled` in project.yml |
| `test_browser_disabled_no_mcp_file` | When `browser.enabled: false`, no `browser.json` is created |
...
| Test | What it verifies |
|------|-----------------|
| `test_browser_port_conflict_auto_assign` | When another running container claims port 9222, effective port becomes 9223; `browser.json` contains `9223`; `.browser-port` contains `9223`; warning emitted |
...
| `test_chrome_resolve_port_from_project` | `--project foo` reads `<cache>/cco/projects/foo/managed/.browser-port` if present |
...
4. **`_generate_browser_mcp`**: generates `managed/browser.json` (in CACHE) with privacy flags and optional `mcp_args`
5. **Compose injection**: managed-dir volume mount (`…/managed:/workspace/.managed:ro`) + `extra_hosts` (between `ports:` and `networks:`)
...
Add managed runtime-state cleanup after `docker compose down`:
...
Add a managed-MCP merge loop over `/workspace/.managed/*.json`, after the existing
project MCP merge. Identical pattern to the global and project merge steps.

**Depends on**: nothing (standalone change, guarded by `[ -d "/workspace/.managed" ]`).
