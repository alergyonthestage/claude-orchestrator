...
Knowledge packs extend the context hierarchy with a reusable document layer. A pack bundles documentation (coding conventions, business overviews, guidelines) from a centralized host directory and injects it into any project's context via `@import`.

**Load path** (within a session):
```
project CLAUDE.md
  └── @.claude/packs.md           ← auto-generated by cco start
        └── @/workspace/.packs/<name>/file.md   ← from mounted source dir
```

| Layer | Container Path | Source |
|-------|---------------|--------|
| Pack mounts | `/workspace/.packs/<name>/` | `global/packs/<name>/pack.yml` → `source:` dir |
| Pack imports | `/workspace/.claude/packs.md` | Auto-generated by `cco start` |

**Activation** — add once to `projects/<n>/.claude/CLAUDE.md`:
```markdown
@.claude/packs.md
```

`packs.md` is regenerated on every `cco start` and gitignored (it lives in `projects/`, which is user-only).

See [CLI.md §4.2](./cli.md) for pack definition format.
