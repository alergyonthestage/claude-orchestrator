...
- Auto memory persists in `user-config/projects/<n>/.cco/claude-state/memory/`
...
| `.cco/claude-state/.gitkeep` | Claude state dir | Mounted to `~/.claude/projects/workspace/`; persists memory and session transcripts |
| `.cco/claude-state/memory/.gitkeep` | Auto memory subdir | Created empty; Claude populates with project insights |
...
| `user-config/projects/<n>/.cco/claude-state/memory/*.md` | Claude Code | Auto memory files (project insights, patterns) |
