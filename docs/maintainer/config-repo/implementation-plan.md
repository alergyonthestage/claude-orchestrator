...
then manifest.yml (needed by install), then vault (independent), then install
commands (depends on manifest.yml), then project install (depends on template
...
## Phase 2 — manifest.yml Management

### 2.1 Create `lib/manifest.sh`
...
```bash
manifest_read()          # Parse manifest.yml, output structured data (uses yaml.sh)
manifest_add_entry()     # Add pack or template entry to manifest.yml
manifest_remove_entry()  # Remove entry from manifest.yml
manifest_refresh()       # Regenerate manifest.yml by scanning packs/ and templates/
manifest_validate()      # Cross-check manifest.yml vs disk (warn on stale entries)
manifest_init()          # Create minimal manifest.yml for a new config dir
```

**YAML writing strategy**: manifest.yml has a simple, flat structure.
Use `cat <<EOF` to regenerate the entire file on each modification.
Do NOT attempt incremental sed-based YAML editing (fragile).
`manifest_refresh()` scans directories and regenerates from scratch.
`manifest_add_entry()` and `manifest_remove_entry()` call `manifest_refresh()`
...
- `cmd_pack_create()`: after creating pack dir + pack.yml, call
  `manifest_refresh "$USER_CONFIG_DIR"` to regenerate manifest.yml
- `cmd_pack_remove()`: after removing pack dir, call
  `manifest_refresh "$USER_CONFIG_DIR"` to update manifest.yml

### 2.3 Add `cco manifest` command

**Files**: `bin/cco` (case statement), `lib/manifest.sh` (command functions)
...
- `cco manifest show` — display manifest.yml contents formatted

### 2.4 Auto-generate manifest.yml on `cco init`

**File**: `lib/cmd-init.sh`

After creating the directory structure, call `manifest_init "$USER_CONFIG_DIR"`.
This generates a minimal manifest.yml:
...
- `tests/test_manifest.sh` (new):
  - Test `manifest_refresh` generates correct YAML from pack dirs
  - Test `manifest_refresh` updates on pack create/remove
  - Test `manifest_validate` warns on stale entries
  - Test `manifest_init` creates minimal manifest.yml

**Commit**: `feat: add manifest.yml management (auto-generated, required manifest)`
...
```bash
cmd_vault_init()     # git init + .gitignore template + manifest.yml if missing
...
3. Validate: `manifest.yml` exists OR `pack.yml` at root (single-pack exception)
4. Read manifest.yml → list available packs
...
- `tests/test_pack_install.sh` (new):
  ...
  - Test install rejects repo without manifest.yml
...
2. Validate manifest.yml (check `templates:` section)
...
- Add `lib/manifest.sh`, `lib/remote.sh`, `lib/cmd-vault.sh` to key files
...
- `cco project install`
- `cco manifest refresh|validate|show`
...
Phase 4 depends on Phase 2 (manifest.yml validation on install).
...
| YAML parser limitations | manifest.yml regenerated fully (no incremental editing) |
