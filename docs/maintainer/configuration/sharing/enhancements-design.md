...
The `.cco/remotes` file is the source of truth; vault remotes are kept in sync.
...
The `.cco/remotes` file uses `<name>=<url>`, one per line. Lines starting with
`#` are comments. Token lines use `<name>.token=<value>`.
...
│     Exclude: .cco/docker-compose.yml, .cco/managed/, .claude/.cco/pack-manifest,
│              .cco/meta, .cco/claude-state/
...
- `.claude/.cco/pack-manifest` (legacy pack tracking, superseded by ADR-14)
