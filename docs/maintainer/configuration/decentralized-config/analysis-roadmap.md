...
built-in), and the doc cutover sweep + `_archive/` are done. **Phase 4 (sharing core) IN PROGRESS —
P4-1/P4-2/P4-3/P4-4 ✅ DONE** (source→DATA · structure-discovery + manifest removed · sync-before-publish ·
2×2 verb wiring [pack import, project export/import, template 2×2, `cco init --template`, project
publish/install/update/internalize REMOVED]); **next = P4-5 legacy teardown** (@local/schema-bridge→index-only,
tier-2 verbs, harness dual-seed/`CCO_*_DIR`) → P4-doc. Resume handoff = `P4-5-handoff-legacy-teardown.md`;
method/phase-map = `Y-handoff-implementation.md` → (T future). Baseline **883/1**.
**Config + sharing design CLOSED; V fully resolved (all 5 clusters); implementation Phases 0–3 closed +
Phase 4 in progress (P4-1…P4-4 done); next = P4-5.**
