...
cco start
  ...
  └── generates .claude/workspace.yml  (knowledge/llms file lists + project summary)

session-context.sh (SessionStart hook)
  └── injects workspace.yml knowledge/llms sections → additionalContext
...
| workspace.yml (`knowledge`/`llms`) | `/workspace/.claude/workspace.yml` | Generated by `cco start`; sections injected by hook |
...
Official framework documentation (llms.txt files) installed via `cco llms install` are mounted read-only at `/workspace/.claude/llms/<name>/`. They are listed in the `llms` section of `.claude/workspace.yml` and injected into context via the `session-context.sh` hook.
...
- **Knowledge packs** — renders the `knowledge` (and `llms`) sections of `/workspace/.claude/workspace.yml` (if present), listing available knowledge files with descriptions
...
graph TD
    subgraph HOST ["HOST — cco start &lt;project&gt;"]
        ...
        H3["generates workspace.yml (knowledge/llms sections + summary) in CACHE → :ro overlay"]
    ...
    subgraph HOOK ["SESSION START HOOK — session-context.sh"]
        ...
        K3[injects workspace.yml knowledge/llms sections → additionalContext]
    end
...
| workspace.yml | `/workspace/.claude/workspace.yml` | `<cache>/cco/projects/<id>/.claude/workspace.yml` (generated, `:ro` overlay) | Hook injection of `knowledge`/`llms` sections (automatic) + on-demand read by `/init-workspace` | `session-context.sh` + Claude Code (via skill) |
...
### workspace.yml knowledge/llms injection

`workspace.yml` is generated at `cco start` time; `session-context.sh` reads its
`knowledge` and `llms` sections at session startup, renders an instructional
preamble around each list, and injects the result into `additionalContext`. This
is automatic and immutable — it does not depend on any `@import` in CLAUDE.md.

The `knowledge` section is a list of `{ path, description }` entries:
```yaml
knowledge:
  - path: /workspace/.claude/packs/<name>/file.md
    description: <description>
```

The hook renders it into an instructional block ("The following knowledge files
are available. Read them proactively when relevant to the current task…").
...
the descriptions in the `knowledge` section of `workspace.yml` guide this decision.
...
4. The `knowledge`/`llms` sections of `/workspace/.claude/workspace.yml` (if present), rendered with their instructional preambles
...
|---------|-------------|-----|
| Pack files not in context | `workspace.yml` `knowledge` section missing or empty | Check `cco start` output confirming `.claude/workspace.yml` was generated; verify pack.yml has `knowledge.files:` |
