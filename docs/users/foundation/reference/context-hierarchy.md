...
cco start
  ...
  └── computes the session context (knowledge/llms file lists + project summary),
      base64-encoded into the CCO_SESSION_CONTEXT env var  (no file — ADR-0042)

session-context.sh (SessionStart hook)
  └── decodes CCO_SESSION_CONTEXT → injects knowledge/llms sections into additionalContext
...
| Knowledge/llms context | injected (no file) | `CCO_SESSION_CONTEXT` env var (base64); the SessionStart hook decodes it into `additionalContext` |
...
Official framework documentation (llms.txt files) installed via `cco llms install` are mounted read-only at `/workspace/.claude/llms/<name>/`. They are listed in the `llms` section of the injected session context (the `CCO_SESSION_CONTEXT` env var — no file) and surfaced via the `session-context.sh` hook.
...
|-------|---------|---------|---------|
| `init-workspace/SKILL.md` | `/init-workspace` | inline | Initialize or refresh project CLAUDE.md from the injected session context |
...
graph TD
    subgraph HOST ["HOST — cco start &lt;project&gt;"]
        ...
        H3["computes session context (knowledge/llms + summary) → CCO_SESSION_CONTEXT env var (base64, no file)"]
    ...
    subgraph HOOK ["SESSION START HOOK — session-context.sh"]
        ...
        K3[decodes CCO_SESSION_CONTEXT → injects knowledge/llms sections → additionalContext]
    end
...
### Session context — knowledge/llms injection

The session context is computed host-side by `lib/session-context.sh` at
`cco start` time and passed to the container as the base64 `CCO_SESSION_CONTEXT`
env var — **there is no `workspace.yml` file** (ADR-0042). The `session-context.sh`
SessionStart hook decodes it, renders an instructional preamble around the
`knowledge` and `llms` lists, and injects the result into `additionalContext`.
This is automatic and immutable — it does not depend on any `@import` in CLAUDE.md.

The `knowledge` section lists each available file with its description, e.g.
`- /workspace/.claude/packs/<name>/file.md — <description>`, and the hook renders
it into an instructional block ("The following knowledge files are available. Read
them proactively when relevant to the current task…").
...
the descriptions in the `knowledge` section of the injected session context guide this decision.
...
### Session context (retires `workspace.yml`)

Computed idempotently at `cco start` and injected as the `CCO_SESSION_CONTEXT`
env var — no file is written into any tree (ADR-0042, INV-2). It provides the
project summary + repos/packs/knowledge/llms the `/init-workspace` skill reads
from its own context. Repo/mount descriptions are authored directly in
`project.yml` (the single structured source, INV-3), not round-tripped through a
generated file.
