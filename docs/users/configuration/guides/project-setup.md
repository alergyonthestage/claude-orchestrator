...
That's it — **no CLAUDE.md edit needed**. Every `cco start` mounts the source directory read-only, records the file list and descriptions in the `knowledge` section of the injected session context (the `CCO_SESSION_CONTEXT` env var — no file), and the `session-context.sh` hook surfaces it in `additionalContext` automatically. Claude sees which files are available and reads them on-demand when relevant. The original files stay in your knowledge repo — zero duplication.
...
2. /workspace/.claude/CLAUDE.md                 ← Project (always loaded)
   ...
   + additionalContext (SessionStart hook):
       ...
       └── session-context knowledge/llms sections  ← Knowledge packs (automatic)
...
**Knowledge packs** are injected by the `session-context.sh` hook into `additionalContext` at startup — Claude sees what files are available and reads them on-demand. No `@import` in CLAUDE.md is required or needed.
