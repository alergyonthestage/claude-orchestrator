...
| `access.claude.entries.agents` | ❌ | enum | `ro` | Content class for `agents/` inside the `repo` / `current` trees, `ro` \| `ask` \| `rw` (mount-enforced per tree). ⚠ **Also selects who gets the coordination guarantee**: because cco enables agent teams, it grants every agent it mounts the tools a teammate needs to deliver (`SendMessage`, the task tools, `ToolSearch`) by mounting a **normalized copy** — a real capability, cross-session messaging, granted deliberately. At `rw` you are authoring the tree, so nothing is projected and cco warns instead. See [agent-teams.md §6](../../integration/guides/agent-teams.md#6-delegation-the-coordination-tools-cco-guarantees) |
