...
cco update                   # Run migrations + discover available updates
...
- **Additive**: New optional config fields → add code-level defaults. No migration, no update action.
...
2. Additive: add code-level default + update base template for new projects
3. Opinionated: update `defaults/global/`; users discover via `cco update --diff`
4. Breaking: create migration in `migrations/{scope}/`, update base template
5. Test: `cco update --project <name>` runs migrations; verify idempotency
