...
- Message levels ([ADR-0059](docs/maintainers/cli/decisions/0059-message-classification-and-the-start-warning-gate.md)): four levels, exactly **one** of which gates a launch. `warn` → `⚠` = a condition of **this session** the user may want to act on, and it **gates `cco start` / `cco new`**; `note` → `note:` = an accepted divergence or an explanation, nothing is wrong; `info`/`ok` = the chronicle of what the command did; a plain `echo … >&2` **inside an interactive prompt** = validation feedback the user is already reading. There is no curated list of gating messages — the rule is a property of the level — so the corollary is what to apply when writing one: **if a message must not stop the launch, it must not be a `warn`.** The badge is the emitter's job: never begin a `warn` body with `⚠` (INV-WG1), and never re-implement the emitter (INV-WG2) — a cloned producer prints identically and captures nothing, so its warning never gates. The capture buffer is a FILE under `${TMPDIR:-/tmp}` (never STATE/DATA/CACHE — INV-S1), because several production `warn`s run inside `$( )` and a shell array would lose exactly those.
