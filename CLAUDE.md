...
- `~/.local/state/cco` (STATE) — machine-local index at `shared/index` (**per-project** logical name → absolute path, v2 nested `project_paths`; identity is the path, name is a per-project label — ADR-0051), session transcripts, memory, update base/meta. On upgrade the index is version-gated and self-healed non-destructively: a host-only gate `die`s if the on-disk index/schema is newer than the binary supports, and a legacy-location index (`<state>/cco/index`) is MERGED into `shared/index` rather than overwritten — never losing registered paths (ADR-0052). The developer execution mode (`cco --dev`, ADR-0060) forks **both** halves of the code identity: these internal buckets **and** the Docker image. ⚠ Isolating the buckets alone was believed to stop a dev build colliding with the published one and does **not** — `IMAGE_NAME` is a single global tag both binaries write, so **the image is where the collision lives**; that belief is what the 2026-08-27 `cco build` incident cost. `--dev-sandbox` / `--dev-sandbox-seed` are superseded aliases of `--dev`.
