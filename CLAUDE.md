...
- `lib/cmd-project-save.sh` / `lib/config-read.sh` — The project half of the config matrix (ADR-0038 + A1 + **A2**): `cco project save` commits `<repo>/.cco/` with a `-- .cco` pathspec on both the staging and the commit, so an already-staged unrelated file is neither committed nor unstaged; a missing/insufficient `.cco/.gitignore` refuses and names the fix rather than being authored (P-C). ⚠ The coverage probe is `git check-ignore **`--no-index`**` and the flag is load-bearing (A2 D13): git consults the INDEX before the ignore chain, so a **tracked** file reports *not ignored* and the refusal prints as its remedy a line already in the file — an unfollowable, permanent refusal. A tracked-but-covered file is a `note` and the save proceeds; the floor that carries this is `_secret_scan_staged`, which sees it as soon as it is also modified. ⚠ A pass is **void** when `.cco/` is ignored wholesale by the repo's ROOT `.gitignore` (A2 D15): every probe reports ignored, `git add -- .cco` stages nothing, and both verbs report success while the config is never saved — the discriminator is the NON-SECRET path `.cco/project.yml`. ⚠ There is **no** ro-mount guard mirroring `_config_save`'s: `git add -- .cco/` on a read-only `.cco` bind returns **0** (git reads the worktree, writes to the rw `.git/`), so the `edit-project+` gate is **policy at the shim**, not mechanism. `config-read.sh` holds the shared renderers for both read questions — `history` (what was saved) and `status` (what is not saved yet). ⚠ Each `status` reproduces its OWN save's commit rule (`.cco/**` vs the personal allowlist), never a bare `git status`, and never writes: `--full` diffs new files with `git diff --no-index`, because `--intent-to-add` would stage them. Both previews report **every** way their save can refuse (A2 D14) — the barrier *and* the secret scan — at rc 0, withholding the `→ … save` hint when one would fire; the scan question is `_secret_scan_paths` asked of the computed set, never of a staged one
