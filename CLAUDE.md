...
- **Session access (capability model, ADR-0036)**: each session resolves three orthogonal knobs — `claude_access` (`none|repo|all`, default `repo`) over the `.claude` authoring trees, `cco_access` (`none|read|edit-project|edit-global|edit-all`, default `none`) over the `.cco`/framework config, and `show_host_paths` (default on). Precedence: CLI (`--claude-access`/`--cco-access`/`--show-host-paths`) > `project.yml` `access:` block > `~/.cco/access.yml` > preset. `--enable-config-edit` is a deprecated alias for `--cco-access edit-project`. When `cco_access ≥ read`, a **whitelisted `cco` runs in-container** (container-operator mode, `bin/cco` `_cco_operator_shim`): read verbs always, path-free write verbs at edit levels; session/image lifecycle + path-resolving + network/credential verbs (`config push`/`pull`, `remote *-token`) stay host-only. Real secret files (`secrets.env`/`*.env`/`*.key`/`*.pem`) are masked from every config mount (only `*.example` visible); tokens/transcripts/memory never mount. Built-ins are presets: config-editor = `all`/`edit-all` (+ `--all`/repeatable `--project` over `<repo>/.cco`), tutorial = `none`/`read`. Resolution + mount generation live in `lib/cmd-start.sh` (`_start_resolve_access`, `_start_generate_compose`); the resolver guard/caller-context in `lib/paths.sh`.
