...
When a feature changes the structure of user-facing config files (`project.yml`, `global/.claude/*`), the update system handles propagation via **3-way merge** (for content changes) or **migrations** (for structural changes).

**`project.yml` is tracked**: content additions (new optional sections) are propagated via 3-way merge automatically when the base template is updated. Structural changes (renamed keys, incompatible schema) still require a migration.
