...
- `lib/cmd-project-save.sh` / `lib/config-read.sh` — The project half of the config matrix (ADR-0038 + A1): `cco project save` commits `<repo>/.cco/` with a `-- .cco` pathspec on both the staging and the commit, so an already-staged unrelated file is neither committed nor unstaged; a missing/insufficient `.cco/.gitignore` refuses and names the fix rather than being authored (P-C). ⚠ There is **no** ro-mount guard mirroring `_config_save`'s: `git add -- .cco/` on a read-only `.cco` bind returns **0** (git reads the worktree, writes to the rw `.git/`), so the `edit-project+` gate is **policy at the shim**, not mechanism. `config-read.sh` holds the shared renderers for both read questions — `history` (what was saved) and `status` (what is not saved yet). ⚠ Each `status` reproduces its OWN save's commit rule (`.cco/**` vs the personal allowlist), never a bare `git status`, and never writes: `--full` diffs new files with `git diff --no-index`, because `--intent-to-add` would stage them
